...
export async function loader() {
    const combinedData = {
        interests: data.interests, // Assuming data is imported from callan-data.json
        comments: fakeCommentsDB // Your fake comments database
    };
...
export async function action({ request }) {
    ...
    const name = body.get("name"); // get name
    const text = body.get("comment"); // get comment
    const id = Date.now(); // use timestamp as unique ID

    // add comment to fake DB
    // to-do: generate unique ID (timestamp?)
    const newComment = { id, name, text };
    fakeCommentsDB.push(newComment);
...
export function CommentsRoute() {
    ...
    const comments = data.comments || []; // Fallback to an empty array if comments is undefined

    return (
        <section className="text-xl mx-auto max-w-4xl text-justify font-serif p-8">
        <div className="space-y-4">
            <h2 className="text-center text-2xl md:text-3xl font-regular mb-4">Add a Comment</h2>
            <Form method="post">
                <input type="text" name="name" placeholder="Your Name" />
                <input type="text" name="comment" placeholder="Enter your comment" />
                <button type="submit">Submit</button>
            </Form>
            <h2 className="text-center text-2xl md:text-3xl font-regular mb-4">Comments</h2>
            {comments.map((comment) => (
                <div key={comment.id}>
                    <ul>
                        
                    <li><p>* {comment.name} @ <em>{new Date(comment.id).toLocaleString()} says:</em></p>
                    <p>"{comment.text}"</p></li>
                    </ul>
                </div>
            ))}
